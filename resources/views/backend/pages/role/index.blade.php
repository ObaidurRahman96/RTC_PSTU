  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h2 class="mb-0"><i class="fa fa-table"> </i> {{ __('backend/admin_setting.role') }}</h2>
      </div>
      <div class="card-body">
        @php
            ...
            <tbody>
              <tr>
                ...
                <td>
                  ...
                  @if(\App\Models\Menu::checkBodyMenu($permission->id, $bodyMenu->in_body) && \App\Models\Admin::where('admin_role','1')->where('status','1')->count('id') > 0)
                ...
                <tr>
                  ...
                  <td>
                    @foreach($permissions->submenus as $key => $permission)
                    @if(\App\Models\Menu::checkBodyMenu($permission->id, $bodyMenu->in_body) && \App\Models\Admin::where('admin_role','2')->where('status','1')->count('id') > 0)
                  ...
                  <tr>
                    ...
                    <td>
                      @foreach($permissions->submenus as $key => $permission)
                      @if(\App\Models\Menu::checkBodyMenu($permission->id, $bodyMenu->in_body) && \App\Models\Admin::where('admin_role','3')->where('status','1')->count('id') > 0)
